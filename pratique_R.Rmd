...
# setwd("/var/data/gluster/volume0/CERISE/01-Espace-de-Partage/SRISE/Pays-De-La-Loire/Partage_R")
...
# import fichiers
chemin_geographie <- "/var/data/gluster/volume0/CERISE/01-Espace-de-Partage/SRISE/Pays-De-La-Loire/Partage_R/Christophe/Geographie/sortie/"
...
# 03-Espace-de-Diffusion/070_Production_lait/7020_Enqu_Annuelle_Laitiere/EAL_2021_resultats_definitifs_septembre_2022
chemin_entree_2021 <- "/var/data/gluster/volume0/CERISE/03-Espace-de-Diffusion/070_Production_lait/7020_Enqu_Annuelle_Laitiere/EAL_2021_resultats_definitifs_septembre_2022/"
...
# Comptes régionaux de l'agriculture : productions et subventions sur les produits - Nouvelles régions (2010-2021) 
...
exploit_otex_dep_reg_fm <- exploit_otex_pdl %>% 
  ...
  mutate(across(contains("part"),arrondi,3))
...
# for working within rows, in combination with rowwise()
...
# rowwise() is just a special form of grouping, 
# so if you want to remove it from a data frame, just call ungroup()
...
relocate
```{r , eval = FALSE}
# the relocate() function to reorder columns. 
# The default behaviour is to move the named column(s) to the first position.
...
## lorsqu'on veut faire un calcul par LIGNE avec des fonctions "vectorielles"
## comme sum, mean ...
...
lait_production_vache_2 <- lait_production_vache_1 %>% 
  ...
  mutate(across(livraison_dep_perc, arrondi, digits=2)) %>% 
...
laits_mensuel_chevre_livr_coll_large_2 <- laits_mensuel_chevre_livr_coll_large_1 %>% 
  ...
  mutate(across(c(livraison_2020,livraison_2021,
                  collecte_2020,collecte_2021,livr2021_evol,coll2021_evol), arrondi, digits=0))
...
# Dans l’exemple suivant, on affecte la valeur (le 1er dep) de valeur_dep_rang1 relative à valeur_dep_rang==1.
# on prend valeur_dep_rang1 correspondant au dep du rang1 (pour valeur_dep_rang == 1) et on affecte la valeur 
# pour l'ensemble des cas 
# jointure de l'ensemble des lignes avec la ligne contenant valeur_dep_rang1 (le 1er dep) ligne par ligne
recolte_bois_dep_reg <-  recolte_bois_dep_reg %>% 
  mutate(commun = 1) 
...
`Comment fai'R pour rajouter ces 0 manquants en début de chaîne ?`
...
# partie entière : nombre entier immédiatement inférieur ou égal au nombre réel en question. 
recolte_bois_dep_reg <- recolte_bois_dep_reg %>%
  group_by(categorie) %>%
