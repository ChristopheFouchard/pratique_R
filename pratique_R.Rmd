...
### Modifier la configuration pour pouvoir installer les packages sur son poste de travail via internet
...
Lancer dans la console RStudio
```{r, eval = FALSE}
# Comment connaître l’adresse du proxy ?
Sys.getenv("http_proxy")  # Pour le proxy HTTP
Sys.getenv("https_proxy") # Pour le proxy HTTPS
```

Puis mettre cette adresse ("...proxy...:8080") dans les variables d'environnement `http_proxy` et `https_proxy`.

Ouvrir le fichier `~/.Renviron` avec la commande suivante :
...
Il existe quelques paramètres qui peuvent s’avérer utiles :  
    ...
    `name =` : permet de nommer la colonne des effectifs obtenus (“effectifs”, par exemple). Par défaut, R la nomme `n`.  

    `sort =` : booléen qui permet, s’il est à TRUE, de classer le tableau par ordre décroissant des valeurs de la colonne `n` (ou “effectifs” si on l’a renommée)
