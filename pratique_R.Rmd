...
Deux remarques sur l'utilisation de cette fonction :
...
tab1_tab2 <- tab1 |>
  ...
  full_join(tab2 |> 
                mutate(in_tab2 = TRUE), 
            join_by(identifiant)) |> 
  mutate(across(contains("in"), ~ replace_na(., FALSE)) |> # Remplacer les NA par FALSE
  mutate(
...
str_replace_all("anormale", "^a", "")  # un 'a' mais seulement en DEBUT DE CHAINE  
## [1] "normale"  
...
- Rechercher des caractères appartenant à des groupes particuliers
  ...
  - une liste de caractères : la séquence `[…]` et son contraire `[^...]`
    str_replace_all("Concrètement", "[oe]", "")  # un 'o' ou un 'e'
    ## [1] "Cncrènt"
