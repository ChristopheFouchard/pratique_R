...
https://www.book.utilitr.org/rproject.html#raccourcis-utiles-pour-l%C3%A9dition-de-code
...
fichier_dep <- "Export Productions Bio - Région - Pays de la Loire.xlsx"
an <- "2021"

agri_bio_vegetal_fr <- read_xlsx(path = paste0(chemin,fichier_nat), 
                                      sheet = "Productions végétales", skip = 0) %>% 
  as_tibble(.name_repair = make_clean_names) %>% 
  filter(annee %in% c(an) & territoire %in% c("National","Pays de la Loire") )
```

https://www.book.utilitr.org/importxl.html#importer-un-fichier-xlsx-avec-le-package-openxlsx
...
https://www.book.utilitr.org/importxl.html#importer-un-fichier-xls-avec-le-package-readxl
...
Source
Cerise : https://rstudio.stats.national.agri
Fichier "DIFFABATVOL_2021.csv", présent dans CERISE/03-Espace-de-Diffusion/060_Productions_viandes_oeufs/6030_Abattage_volailles_lapins/60304_Volailles/ABATVOL_2021
...
```{r , eval = FALSE}
# 1- Dans ce tableau ne conserver que l'année, le type (VB et VNB), les totaux départementaux et régionaux
lait_production_vache_type <- lait_production_vache_1 %>%
  group_by(type) %>%
  mutate(livraison_reg=sum(colldiff, na.rm=TRUE)) %>%
  group_by(type,dep_prod) %>% 
  mutate(livraison_dep=sum(colldiff, na.rm=TRUE)) %>%
  distinct(annee,type,livraison_reg,livraison_dep)
...
Pour plus de précisions, consulter la fiche https://www.book.utilitr.org/jointures.html qui présente un certain nombre de règles et de bonnes pratiques sur les jointures.
...
https://www.book.utilitr.org/tidyverse.html#le-tibble-un-data.frame-am%C3%A9lior%C3%A9
...
https://www.book.utilitr.org/tidyverse.html#encha%C3%AEner-les-manipulations-avec-lop%C3%A9rateur-pipe
