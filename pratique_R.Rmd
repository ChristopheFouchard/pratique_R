...
<center>![](images/Cerise_ouvrir_session_R.png)</center>

puis chosir la version de R à travers la fenêtre :

<center>![](images/Cerise_nouvelle_session_R.png)</center>


### Pour récupérer un chemin pour l'utiliser dans un programme R ?
...
# ou setwd("~/CERISE/01-Espace-de-Partage/SRISE/Pays-De-La-Loire")
# setwd("/var/data/gluster/volume0/CERISE/01-Espace-de-Partage/SRISE/Pays-De-La-Loire")

```
Le chemin peut-être aussi copié depuis la console (fenêtre en bas à gauche).

### Pour se déplacer rapidement vers un autre répertoire ?
...
### Pour copier un fichier ou un dossier via R
...
```
### Pour transférer des fichiers entre son poste de travail et Cerise

#### Pour télécharger un fichier depuis Cerise
...
#### Pour téléverser un fichier vers Cerise
...
Pour gagner du temps, il est important d'utiliser les raccourcis pour les commandes répétitives. 
    ...
    `Ctrl` + `Shift` + `M` : insère le pipe `%>%`
     Plusieurs verbes (ou fonctions) peuvent facilement être combinés en utilisant l'opérateur `%>%` (pipe) qui permet d'enchaîner les instructions.
    ...
    `Alt` + `-` : insère la flèche d'assignation <- (il faut utiliser le - du pavé numérique, pas celui du 6)
    `Alt`+ `6` (Windows) : insére l'opérateur assignation `<-`
    `Ctrl`+`Entrée` : exécute le code sélectionné (ou la commande ou se trouve le curseur si rien n'a été sélectionné)
    ...
    `Ctrl` + `I` : indente automatiquement le code sélectionné (ou la commande ou se trouve le curseur si rien n'a été sélectionné)
    Ctrl + Shift + C : bascule en commentaire le code sélectionné (ou la ligne où se trouve le curseur si rien n'a été sélectionné)
    Tab ou Ctrl + Espace : ouvre les propositions d'autocomplétion (pour les noms de fonctions ou d'objets, et pour les chemins à l'intérieur de guillemets)

    F1 : sur une fonction R, affiche l'aide de la fonction (si le package est chargé). On peut aussi utiliser ?nom_fonction (ou ??nom_fonction). La documentation de la fonction apparaîtra dans le panneau en bas à droite de RStudio.
    F2 : sur une fonction, affiche le code de la fonction (sur une fonction personnalisée uniquement si le script contenant la définition de la fonction est ouvert)
...
### A quoi sert le fichier ".RData" - Faut-il le conserver ?
...
### Installation de Rtools en cas de difficulté
...
Certains packages ne peuvent s'installer sur CERISE qu'avec des versions plus anciennes (pour cause de bibliothèque système absente ou trop ancienne sur la version CentOS de CERISE).
...
Dans le cas où on ne sait pas quelles sont les colonnes dont on a besoin, il est conseillé de commencer par importer un petit nombre de lignes (1 000 ou 10 000) afin d’étudier les données et de choisir ensuite les colonnes à importer. 

### Importer des fichiers RDS


```{r, eval=FALSE}
# Source : Les données sont issues du recensement agricole 2020, avec une comparaison à celui de 2010.
# Chargement des données
# RA2020 PDL DONNEES INDIVIDUELLES
chemin_pdl_20 <- "S:/Fichiers source/RA 2020_déf/BASES_RA2020_data_def_15avril2022/RDS/"
annee_ra_2020            <- 2020

exploit_pdl_2020 <- readRDS(file = paste0(chemin_pdl_20,"RA2020_EXPLOITATIONS_R52_220415_modif_27juin2022.rds") ) %>% 
...
Source
Cerise - Fichier "DIFFABATVOL_2021.csv", présent dans CERISE/03-Espace-de-Diffusion/060_Productions_viandes_oeufs/6030_Abattage_volailles_lapins/60304_Volailles/ABATVOL_2021
...
**Il est plus simple de sélectionner des colonnes avec `fread()` qu'avec les fonctions du *package* `readr`**. Il est donc recommandé d'utiliser `fread()` lorsque vous souhaitez sélectionner facilement les colonnes à importer.
:::
...
agri_bio_vegetal_fr_tab1 <- agri_bio_vegetal_fr %>%   
  ...
  select(annee,territoire,nombre_de_producteurs,surface_bio_et_en_conversion_en_ha,surface_en_conversion_en_ha,surface_bio_en_ha,
...
Voici comment on peut utiliser `separate` pour scinder `com` en deux nouvelles colonnes `code_dep` et `code_com`. La colonne `com` disparait, car par défaut `separate` supprime la colonne scindée. Si on veut la conserver, il faut ajouter l'option `remove = FALSE`.
...
Voici un exemple où l'on regroupe le code commune Insee et le nom officiel de la commune, avec " - " comme séparateur. Les colonnes `com` et `ncc` disparaissent, car par défaut `unite` supprime les colonnes regroupées. Si on veut les conserver, il faut ajouter l'option `remove = FALSE`.
...
agri_bio_vegetal_tab1_final <- agri_bio_vegetal_tab1_final %>% 
  select(c("annee","vegetal","Loire-Atlantique","Maine-et-Loire", "Mayenne","Sarthe","Vendée","Pays de la Loire","National"))
...
agri_bio_vegetal_fr_tab1_car <- agri_bio_vegetal_fr_tab1 %>% 
  mutate(across(c(nombre_de_producteurs,surface_bio_et_en_conversion_en_ha,surface_en_conversion_en_ha,surface_bio_en_ha,
...
produits_a17_percent <- produits_a17_large %>% 
   ...
   mutate(across(where(is.numeric),arrondi,digits=2))
...
# [1] "Pommes de table"   "Pommes de terre (hors féculière)" "Pommes à cidre et à jus"  "Pommes (sans précision)"  
...
agri_bio_vegetal_fr_cultures <- agri_bio_vegetal_fr %>%   
    ...
    sous_groupe_de_productions %in% c("Légumes secs","Légumes frais","Plantes à parfum, aromatiques et médicinales") ~ "4_Légumes, fruits et PPAM",
    groupe_de_productions %in% c("Fruits (yc à coque)") & sous_groupe_de_productions %in% c("Toutes") ~ "4_Légumes, fruits et PPAM",
    sous_groupe_de_productions %in% c("Viticulture") ~ "5_Vigne",
    sous_groupe_de_productions %in% c("Cultures fourragères","Surfaces toujours en herbe") ~ "6_Fourrage et STH",
    groupe_de_productions %in% c("Autres surfaces") & sous_groupe_de_productions %in% c("Toutes") ~ "7_Autres",
    TRUE ~ "autre cas - doublon")
  )
...
volailles_pdl$espece_regrpt1 <- fct_collapse(volailles_pdl$libelle_espece,
                                                "b11-poulets" =  c("Poulets (yc coquelets)","Chapons, poulardes",
...
La fonction 'cut()' sert à créer des classes : seq(0,120,5) => de 0 à 120 par paliers de 5
         
'right = FALSE' pour fermé à gauche, ouvert à droite [inf,sup)]
...
Par exemple, si on veut avoir la médiane de la variable P14_POP, pour chaque ZE et chaque région :
```{r , eval=FALSE}
...
age_moyen <- exploitants_2010 %>% 
  ...
  bind_rows(mo_2020 %>% 
            mutate(annee = annee_ra_2020,
                  anais = replace(anais, which(anais == 0 | anais == 9999), NA), # il existe des valeurs nulles d'années de naissance (2 dans la Manche !)
...
# calcul moyenne semestrielle
# avec rowwise() (fonction dplyr) et c_across() for working within rows, in combination with rowwise()
...
relocate
```{r , eval = FALSE}
# the relocate() function to reorder columns. The default behaviour is to move the named column(s) to the first position.
...
eff_volailles_evol_part <- eff_volailles_evol %>% 
  ...
  relocate(an_2021_pdl,evol_2021_2020_pdl,an_2021_fm,evol_2021_2020_fm,part_pdl_france_2021, .after = espece )
...
lait_chevre_dep_2021_donnees <- lait_chevre_dep3 %>% 
  ...
  relocate(producteurs_2020,livraisons_2020,producteurs_2021,livraisons_2021, .after = livraisons_2019)
...
## lorqu' on veut faire un calcul par LIGNE avec des fonctions "vectorielles"
## comme sum , mean ...
## il faut indiquer  à R que l'on veut un calcul par ligne 
...
```{r , eval = FALSE}
# 1- Dans ce tableau ne conserver que l'année, le département de la production, le type (VB et VNB), les totaux départementaux et régionaux
...
# 2- Réaliser un tableau similaire pour l'ensemble des vaches (sans distinguer VB et VNB, en indiquant type="vache")
...
# 3- Regrouper les 2 tableaux agrégés : tableau pour l'ensemble des vaches + tableau distinguant VB et VNB
...
# agrégation données
emploi_salarie <- bind_rows(a_agri_long, b_iaa_long,cz_ind_tot_long,d_constr_long,e_commerce_long,fz_serv_march_tot_long,fz_serv_non_march_tot_long)
...
![Transformation *wide to long*](./images/widetolong.png)

* **Transformation *long to wide* ** : 

![Transformation *long to wide*](./images/longtowide.png)
...
agri_bio_vegetal_fr_tab1_long <- agri_bio_vegetal_fr_tab1_car %>%   
  pivot_longer(cols=c("nombre_de_producteurs","surface_bio_et_en_conversion_en_ha","surface_en_conversion_en_ha","surface_bio_en_ha",
...
# crée variables "trimestre"(modalités 'x1','x2','x3','x4') et "annee" à partir de c('x4t2018','x1t2019'...)  
produits_long <- produits %>%   
             pivot_longer(cols = c('x4t2018','x1t2019','x2t2019','x3t2019','x4t2019','x1t2020','x2t2020','x3t2020','x4t2020',
                                   'x1t2021','x2t2021','x3t2021','x4t2021','x1t2022','x2t2022','x3t2022'), 
...
itavi_2022_2021_espece <- itavi_2022_2021 %>%   
             pivot_longer(cols = c(poulet_standard,poulet_label_rouge,dinde,pintade,poule_pondeuse,canard_rotir,canard_gras,lapin,
...
# Calculer l'évolution de la production de lait et de la collecte de lait entre 2020 et 2021, en hl et en %
laits_mensuel_chevre_livr_coll_large_2 <- laits_mensuel_chevre_livr_coll_large_1 %>% 
  ...
  mutate(across(c(livr2021_evol_perc,coll2021_evol_perc), arrondi, digits=2)) %>% 
  mutate(across(c(livraison_2020,livraison_2021,collecte_2020,collecte_2021,livr2021_evol,coll2021_evol), arrondi, digits=0))
...
<center>![](images/left_join.png)</center>
...
Parfois, il peut être utile de réaliser une jointure de la table avec elle-même pour affecter la valeur d'une variable à une autre variable. Dans l'exemple suivant, on affecte la date d'installation qui est relative au chef d'exploitation au co-exploitant.
```{r , eval = FALSE}
# Installés depuis 2010
# anchef	Année de début de fonction du chef
# on prend l'année d'installation anchef du chef (statutdirig == 1) et on affecte la valeur dans anchef_co 
# pour l'ensemble des chefs et co-exploitants (même nom_dossier)
mo_2020_install2010 <-  mo_2020 %>% 
  left_join(mo_2020 %>% 
              select(nom_dossier,statutdirig_y=statutdirig,anchef_co=anchef) %>%
              filter(statutdirig_y == 1),
            by="nom_dossier") %>% 
  select(-statutdirig_y) %>% 
  mutate(age=2020-as.numeric(anais),
         install2010 = (!is.na(anchef_co) & (anchef_co >= 2010) ) )
```
### Vérification de doublons de lignes 

Cette vérification peut permettre de déterminer les varaibles servant d'identifiant commun à 2 tables.
...
<center>![](images/full_join.png)</center>
...
nb_exploit_vaches_dep_reg <- nb_exploit_vaches_dep %>%
  ...
  adorn_totals(where = c("row"), name="Pays de la Loire" ) %>% 
  mutate(across(c(part_expl,part_vaches),arrondi,digits=2))
...
Pour arrondir systématiquement à l'excès au-delà de 5, il convient d'utiliser une fonction spécifique (à placer en début de programme)

Les noms des colonnes peuvent être appelés avec la syntaxe `nomtable$nomvariable`. L’indexation des colonnes de tables (le “$”) permet d'appeler une variable

```{r fonctions arrondi, eval = TRUE}
# Fonction pour gérer les arrondis de .5 
...
# round(3.5,digits=0) donne 4 tout comme round(4.5,digits=0)
# L"'"aide de cette fonction indique que les arrondis respectent la norme IEC 60559 
...
a <- round(3.5,digits=0)
a1 <- arrondi(3.5,digits=0)
a
a1
b <- round(4.5,digits=0)
b1 <- arrondi(4.5,digits=0)
b
b1
x1 <- c(3.5,4.5,1.35,1.45,1.46,1.5,1.6,2.5,2.6)
round(x1,0)
arrondi(x1,0)
round(x1,1)
arrondi(x1,1)
...
lait_vache_reg_dep_2021_donnees_secret_1 <- lait_vache_reg_dep_2021_donnees %>% 
  ...
  mutate(livraisons_2021=ifelse(as.numeric(producteurs_2021) > 0 & as.numeric(producteurs_2021) < 3,"Secret",livraisons_2021) ) %>%
  mutate(producteurs_2021=ifelse(as.numeric(producteurs_2021) > 0 & as.numeric(producteurs_2021) < 3,"Secret",producteurs_2021) ) %>% 
...
```{r, eval=FALSE}
> rp_individu_pdl_age_actif <-  rp_individu_pdl_var %>% 
+   mutate(age=case_when(agerevq %in% c("020","025","030","035","040","045","050","055","060") ~ "a_20_64_ans",
+                        TRUE ~ agerevq),
