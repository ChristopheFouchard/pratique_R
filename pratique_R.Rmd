...
Pour exporter un tableau de données dans R vers un fichier au format texte délimité csv (*comma separated values*), on peut utiliser `write_csv` ou `write_csv2`.
...
On peut spécifier précisément le comportement de dégroupage de `summarise` en lui fournissant un argument supplémentaire `.groups` qui peut prendre notamment les valeurs suivantes :
...
- `names_from` : un vecteur contenant le nom de la (ou des) colonne(s) qui donne(nt) les noms des nouvelles colonnes à créer ;
...
Il peut arriver que certaines variables soient absentes pour certaines observations. Dans ce cas l'argument `values_fill` permet de spécifier la valeur à utiliser pour ces données manquantes (par défaut les valeurs absentes sont indiquées par des `NA`).
...
Ces 3 sites servent de source pour alimenter le présent document : 
...
Une fonction a un *nom*, elle prend en entrée entre parenthèses un ou plusieurs *arguments* (ou *paramètres*), et retourne un *résultat*. Elle se termine souvent par la fonction `return()`. 
