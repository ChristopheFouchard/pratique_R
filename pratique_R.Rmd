...
- une vue des scripts et des données (en haut à gauche),  
- une console (en bas à gauche),  
- l’espace de travail et l’historique des commandes (en haut à droite),  
- un navigateur offrant une vue des répertoires et fichiers, des graphiques, des packages et de l'aide (en bas à droite).
...
write.xlsx(liste_tableaux, file = paste0(chemin_sortie, fichier_sortie))
...
# Valeurs manquantes
# On retourne toutes les lignes du dataframe qui contiennent au moins une valeur manquante (NA) dans n'importe quelle colonne.
...
recolte_bois_dep_reg <- recolte_bois_dep_reg |>
  mutate(valeur_dep_percent_reg = valeur_dep / valeur_reg * 100,
         ...
         .after = dep) |> 
  mutate(across(contains("percent"), \(x) round_half_up(x, digits = 3)))
...
### comparaisons entre `recode()`, `if_else()` et `case_when()`

case_when(), comparativement à recode() et if_else()
```{r , eval = FALSE}
...
exploit_lait2_production <- exploit_lait2_production |>
  mutate(type_regroupement_2 = if_else(type %in% c("VB","VNB"), yes = "Vaches", no = type),
         type_regroupement_2 = if_else(type %in% c("CB","CNB"), yes = "Chevres", no = type_regroupement_2),
         type_regroupement_2 = if_else(type %in% c("BB","BNB"), yes = "Brebis", no = type_regroupement_2))
...
exports_pdl_2020_pays_vol_vivantes <- filtrer_an_type_produit(exports_pdl_fr_an_pays_vol, 2020, 'Volailles vivantes et œufs')
exports_pdl_2021_pays_vol_vivantes <- filtrer_an_type_produit(exports_pdl_fr_an_pays_vol, 2021, 'Volailles vivantes et œufs')
```
...
```{r , eval = FALSE}
# L'utilisation de rowwise permet d'appliquer les opérations ligne par ligne.
...
- `cols` : cols = noms des colonnes à pivoter,  
...
En effet, il existe plusieurs manières différentes d’associer des données qui déterminent directement la clé à utiliser :
    ...
    - 1 to m (one to many) : 1 observation côté A correspond à plusieurs observations côté B. Une observation côté A (par exemple d’une unique région) correspond à plusieurs observations côté B (par exemple des différents départements constituant la région). La clé serait alors la région et des lignes de A se retrouveraient alors dupliquées.  

    - m to 1 (many to one) : plusieurs observations côté A correspondent à 1 observation côté B. Plusieurs observations côté A (le nombre d'observations des différents départements d’une unique région) correspondent à une seule observation côté B (les données de la région composée par ces départements). La clé serait alors la région et des lignes de B se retrouveraient alors dupliquées.  
...
comparer_tables <- function(tab1, tab2, identifiant) {
  ...
  comparaison <- tab1 |>
    ...
    mutate(
      ...
      across(starts_with("in_"), ~replace_na(., FALSE)),
...
Rajouter des caractères en début ou en fin de chaîne pour respecter un format :   
...
# starts_with("nb_tetes") : Cette sélection s'applique uniquement aux colonnes dont les noms commencent par "nb_tetes".
...
# \\d : Correspond à un chiffre (0-9). Le double antislash (\\) est utilisé 
# pour échapper le caractère \ dans les chaînes de caractères en R.
...
calculer_montants_par_departement <- function(annee) {
  ...
  montants_pdl <- fic_benef |>
    ...
    summarise(
      ...
      dont_paiement_additionnel_jeunes_agriculteurs = sum(.data[[paste0("pja_", an)]] / 1000, na.rm = TRUE),
      dont_paiement_redistributif = sum(.data[[paste0("pred_", an)]] / 1000, na.rm = TRUE),
      ...
      nb_droits_a_paiement_de_base = sum(.data[[paste0("nb_dpb_", an)]], na.rm = TRUE),
      ...
      dont_aide_bovine_post_2023 = if (an == 23) {
        sum(.data[[paste0("abh_", an)]] / 1000, na.rm = TRUE)
      } else {
        NA_real_
      }, # Nouvelle aide PAC 2023 aide bovine, hexagone
      dont_aide_veaux_sous_mere_et_veaux_bio = sum(.data[[paste0("vm_", an)]] / 1000, na.rm = TRUE),
      dont_aide_bovin_allaitant = if (an == 23) {
        NA_real_
      } else {
        sum(.data[[paste0("aba_", an)]] / 1000, na.rm = TRUE)
      }, # n'existe plus en 2023
      dont_aide_bovin_laitier = if (an == 23) {
        NA_real_
      } else {
        sum(.data[[paste0("abl_", an)]] / 1000, na.rm = TRUE)
      }, # n'existe plus en 2023
      dont_aide_ovins_et_caprins = sum(.data[[paste0("ovicap_", an)]] / 1000, na.rm = TRUE),
      dont_aide_ovins = sum(.data[[paste0("ao_", an)]] / 1000, na.rm = TRUE),
      dont_aide_caprins = sum(.data[[paste0("ac_", an)]] / 1000, na.rm = TRUE),
      montant_aides_couplees_vegetales = sum(.data[[paste0("acv_", an)]] / 1000, na.rm = TRUE),
      # prot_23 = legf_ds_23 + legf_23 + sgram_23
      dont_aide_proteines_vegetales = sum(.data[[paste0("prot_", an)]] / 1000, na.rm = TRUE),
      dont_aide_legumineuses_fourrage_deshy_semence = if (an == 23) {
        sum(.data[[paste0("legf_ds_", an)]] / 1000, na.rm = TRUE)
      } else {
        sum((.data[[paste0("dh_", an)]] +
               .data[[paste0("slegf_", an)]]) / 1000, na.rm = TRUE)
      }, # changement en 2023
      dont_aide_legumineuses_fourrage_deshy = if (an == 23) {
        NA_real_
      } else {
        sum(.data[[paste0("dh_", an)]] / 1000, na.rm = TRUE)
      }, # n'existe plus en 2023
      dont_aide_semences_legumineuses_fourrageres = if (an == 23) {
        NA_real_
      } else {
        sum(.data[[paste0("slegf_", an)]] / 1000, na.rm = TRUE)
      }, # n'existe plus en 2023
      dont_aide_legumineuses_fourrageres = sum(.data[[paste0("legf_", an)]] / 1000, na.rm = TRUE),
      dont_aide_semence_graminees = sum(.data[[paste0("sgram_", an)]] / 1000, na.rm = TRUE),
      dont_aide_proteagineux = if (an == 23) {
        NA_real_
      } else {
        sum(.data[[paste0("ap_", an)]] / 1000, na.rm = TRUE)
      }, # n'existe plus en 2023
      dont_aide_soja = if (an == 23) {
        NA_real_
      } else {
        sum(.data[[paste0("soj_", an)]] / 1000, na.rm = TRUE)
      }, # n'existe plus en 2023
      dont_autres_aides_vegetales = sum(.data[[paste0("veg_", an)]] / 1000, na.rm = TRUE),
      dont_aide_maraichage = if (an == 23) {
        sum(.data[[paste0("maraich_", an)]] / 1000, na.rm = TRUE)
      } else {
        NA_real_
      }, # Nouvelle aide PAC 2023
      dont_aide_chanvre = sum(.data[[paste0("chv_", an)]] / 1000, na.rm = TRUE),
      montant_ichn = sum(.data[[paste0("ichn_", an)]] / 1000, na.rm = TRUE),
      montant_assurance_recolte = sum(.data[[paste0("aras_", an)]] / 1000, na.rm = TRUE),
      montant_bio = sum(.data[[paste0("bio_", an)]] / 1000, na.rm = TRUE),
      dont_montant_bio_conversion = sum(.data[[paste0("bio_cab_", an)]] / 1000, na.rm = TRUE),
      dont_montant_bio_maintien = sum(.data[[paste0("bio_mab_", an)]] / 1000, na.rm = TRUE),
      montant_maec = sum(.data[[paste0("maec_", an)]] / 1000, na.rm = TRUE),
      montant_aide_2d_pilier = sum(montant_ichn,
                                   montant_assurance_recolte,
                                   montant_bio,
                                   montant_maec,
                                   na.rm = TRUE
      ),
    ...
    ungroup() |>
    relocate(montant_aide_totale, .before = montant_aide_1er_pilier) |>
    relocate(montant_aide_2d_pilier, .before = montant_ichn) |>
    adorn_totals(
      where = "row",
      name = "Ensemble"
    ) |>
...
# na.string = "NA" : spécifie comment les valeurs manquantes doivent être représentées dans le fichier Excel (ici, comme "NA")
write.xlsx(dfs_nb_tetes_par_groupe_production_clean,
  file = paste0(chemin_sortie, fic_bio_anim_nb_tetes_dep_pdl_part_reg),
  na.string = "NA"
)
...
Un *vecteur* est un ensemble de 1 ou plusieurs éléments du même type.
...
On peut aussi faire un test logique avec is.numeric(x) ou is.character(x) par exemple.  
...
Pour obtenir le nombre de lignes et de colonnes dans *data frame* on utilise les fonctions `nrow()` et `ncol()`.  
...
bio_pdl_nomenclature_pb_na <- bio_pdl_nomenclature |> 
  mutate(libelle_ab_regroupt = if_else(libelle_ab_2 == "Pommes", yes = libelle_ab_2, no = libelleonab))
...
# error in `if_else()`: Can't combine `true` <character> (corresponnd à "Secret") and `false` <double> (corresponnd à .x)
...
recolte_bois_dep_reg <- recolte_bois_dep_reg |>
  ...
  mutate(valeur_dep_rang = rank(-valeur_dep),
         .before = 1) |>
  mutate(valeur_dep_rang1_a = if_else(valeur_dep_rang == 1, yes = libdep, no = ""),
