...
<!-- 
largeur max colonne : 116 caractères
-->


Ce **"guide pratique R"** est un manuel expliquant l'utilisation du langage de programmation **R** pour la **manipulation des données**.
...
https://ssm-agriculture.github.io/formation-cerise/

### Version de R

La version de R par défaut est la 4.4.1. La version précédente (4.2.3) est toujours disponible.
...
<!-- 
Pour centrer une image avec contrôle de largeur
![*Légende optionnelle*](chemin/vers/image.extension){fig-align="center" width="XX%"}
L'attribut fig-align ne doit pas avoir d'espaces autour du signe égal 
et doit être entre accolades après le chemin de l'image
-->

![](images/Cerise_version_R.png){fig-align="center" width="50%"}
...
![](images/Cerise_ouvrir_nouvelle_session_R.png){fig-align="center" width="50%"}

puis choisir de l'associer à un projet ou à un dossier :

![](images/Cerise_nouvelle_session_R_directory.png){fig-align="center" width="50%"}
...
![](images/Cerise_nouvelle_session_R_directory_pdl.png){fig-align="center" width="50%"}

Puis choisir la version de R à travers la fenêtre :

![](images/Cerise_nouvelle_session_version_R.png){fig-align="center" width="50%"}
...
Dans la fenêtre de parcours des dossiers (fenêtre en bas à droite dans RStudio Server), choisir `Plus`  
puis `Copier le chemin du dossier dans le presse-papiers`.  


On peut aussi utiliser `Plus` puis `Définir comme répertoire de travail`.  
```{r}
# setwd("~/CERISE/01-Espace-de-Partage/SRISE/Pays-De-La-Loire/Partage_R")

```
Le chemin peut alors être copié depuis la console (fenêtre en bas à gauche).
...
Il faut naviguer dans les dossiers jusqu'au fichier et ensuite sélectionner celui-ci pour pouvoir le télécharger, puis aller dans Plus > Exporter

#### Comment téléverser un fichier vers Cerise

Sur votre PC en local, faire un zip qui contient les fichiers à uploader (même si le fichier est unique). Cela permet d’affecter
les “bons droits” sur ces fichiers (notamment qu’ils soient modifiables par d’autres agents). Son contenu sera ensuite automatiquement dézippé sous Cerise.    
...
Une fois placé dans le dossier de dépôt du fichier, choisir `Télécharger` (Télécharger des fichiers vers le serveur)
...
La commande `rm(list = ls())` détruit les pointeurs vers les morceaux de mémoire utilisés. 
La commande `gc()` permet ensuite de libérer manuellement la mémoire occupée par des objets inutilisés (à lancer après la suppression d’un objet volumineux).
...
Les projets permettent de ne pas avoir à spécifier un chemin complet vers un fichier (comme `D:\\...\\data\\donnees.xlsx`) mais un chemin relatif au dossier du projet (juste `data/donnees.xlsx` si le fichier se trouve dans un sous-dossier `data`.)
...
Les problèmes d’installation sont relativement souvent liés à un problème dans l’installation des dépendances. Une dépendance est un package dont a besoin le package qu’on est en train d’installer et qui peut donc s'installer en même temps.  
...
Il est aussi possible d'installer un _package_ depuis une archive compressée (fichiers ".zip" pour les binaires Windows ou ".tar.gz" pour les _packages_ _source_). Pour indiquer que la source est un fichier local, il est obligatoire de spécifier `repos = NULL`. repos (pour repository) correspond au dépôt, au  lieu de stockage des packages.    
...
### Modifier la configuration pour pouvoir installer les packages via internet  
...
Une fois le package installé, il faut le "charger" avant de pouvoir utiliser les fonctions qu'il propose. 
...
Le package `fs` offre des fonctionnalités pour manipuler les fichiers et les répertoires, comme :  
`dir_ls()` : Lister les fichiers dans un répertoire.  
`file_move()` : Déplacer des fichiers.  
`file_copy()` : Copier des fichiers.  
`file_delete()` : Supprimer des fichiers.  
`dir_create()` : Créer des répertoires.  
...
Pour importer dans `R` des données issues de tableurs avec l'extension`xlsx`, il est recommandé d'utiliser la fonction `read_xlsx()` du _package_ `openxlsx2`.

Voici les principaux arguments et options de `read_xlsx()` :

| Argument         | Valeur par défaut     | Fonction                                                                                   |
|------------------|-----------------------|--------------------------------------------------------------------------------------------|
| `file`       | Aucune                | Chemin d'accès vers un fichier `xlsx` à importer           |
| `sheet`          | `1`                     | Onglet à importer. Soit le nom de l'onglet (entre guillemets), soit un numéro de l'onglet |
| `start_row`      | `1`                     | Ligne à partir de laquelle les données sont importées.     |
| `col_names`      | `TRUE`                  | Si `TRUE`, la première ligne de données sera utilisée comme nom de colonnes   |
|
...
eff_volailles <- read_xlsx(file = path(chemin_entree, fichier_entree), 
                           sheet = "Eff. volailles") |> 
  as_tibble(.name_repair = make_clean_names) |> 
  filter(region %in% c("52  -  Pays de la Loire",
                       "98  -  Total France métropolitaine"))
...
agri_bio_vegetal_fr <- read_xlsx(file = path(chemin,fichier_nat), 
                                 sheet = "Productions végétales") |> 
  as_tibble(.name_repair = make_clean_names) |> 
...
laits_reg_dep_2021 <- read_xlsx(file = path(chemin_entree_2021, fichier_entree_liv_reg_dep), 
                                sheet = "Sheet 1") |> 
  as_tibble(.name_repair = make_clean_names) |> 
...
### Importer des fichiers plats (csv, txt)

L'utilisateur souhaite importer dans `R` des données stockées sous forme de fichiers plats (formats `.txt`, `.csv`, `.tsv`).  
Les "fichiers plats" sont des fichiers texte qui stockent des données avec des délimiteurs pour séparer les colonnes.  
Ces fichiers contiennent des données tabulaires, c'est-à-dire des données organisées sous forme de tableau, avec en lignes les observations et en colonnes les variables. 
...
### Importer des fichiers parquet 

Le format **Parquet** est un format de stockage particulièrement adapté aux données volumineuses.  

Le package **`arrow`** permet de lire ces fichiers de manière performante, sans devoir tout charger en mémoire. 

On privilégiera l’approche `open_dataset()` lorsque la volumétrie des données est importante.

L'ordre optimal pour l'import pour minimiser la charge en mémoire est :
```{r, eval = FALSE}
donnees <- open_dataset("chemin/vers/fichier.parquet") |>
  filter()    # 1. Filtrer d'abord (réduit les lignes)
  select()    # 2. Sélectionner les colonnes (réduit les colonnes)
  collect()   # 3. Charger en mémoire
```

L'appel à `collect()` ne charge en mémoire que les sous-ensembles filtrés, ce qui est très utile pour les gros volumes. `collect()` renvoie le résultat du traitement sous la forme d’un `tibble`.

```{r, eval = FALSE}
library(arrow) # pour lire les fichiers parquet
library(dplyr) # pour la manipulation de données
library(fs)    # pour la gestion des chemins de fichiers

# import fichiers ----
# source : insee, flores
chemin_entree <- "~/CERISE/03-Espace-de-Diffusion/150_Sources_externes/15070_INSEE/FLORES/1_TABLES/"
chemin_an <- "2023/"
fichier_entree_etab <- "flores_etab_2023.parquet"

# méthode conseillée pour des fichiers volumineux
# Établir la connexion aux données
donnees_flores <- open_dataset(sources = path(chemin_entree,
                                              chemin_an,
                                              fichier_entree_etab))

# Afficher les noms de colonnes 
names(donnees_flores)

# Nom et type des variables
donnees_flores

# Obtenir les valeurs distinctes et triées de 'reg' (exécution lors du collect)
liste_reg <- donnees_flores |>
  distinct(reg) |>
  arrange(reg) |>
  collect()

# Conversion en vecteur simple
reg_vecteur <- liste_reg$reg
print(reg_vecteur)

# Variante avec pull() pour obtenir directement un vecteur
liste_a17 <- donnees_flores |>
  select(a17) |>
  distinct() |>        
  collect() |>
  pull(a17)
...
# Définir une requête optimisée
# Filtrer d'abord pour réduire le volume
# Sélectionner uniquement les colonnes nécessaires
requete <- donnees_flores |>
  filter(reg == "52",
         a17 %in% c("C1")) |> # réduire d'abord le nombre de lignes
  select(reg, a17, siren, eff_etab) # limiter les colonnes 

# Récupérer le résultat sous forme d'un tibble
iaa_etab_pdl <- requete |> 
  collect()
...
https://book.utilitr.org/03_Fiches_thematiques/Fiche_import_fichiers_parquet.html  
https://book.utilitr.org/03_Fiches_thematiques/Fiche_arrow.html  
https://ssm-agriculture.github.io/formation-R-perf-06-parquet/  
https://ssm-agriculture.github.io/formation-R-perf-06-parquet/#/comment-bien-utiliser-des-fichiers-partitionn%C3%A9s-avec-arrow-12  
...
comdepuis1943_com2022 <- read_sas("S:/ETUDES/memento/entree/geographie/geo2022/fcodcomm.sas7bdat")
...
### Importer des fichiers avec en-tête sur 2 lignes
...
# et de renvoyer une nouvelle liste résultante.
list_eff_annees <- map(annees, function(annee) {
  ...
  # return facultatif. Certains considèrent que l'ajouter permet de rendre le code plus lisible. 
  return(df)
})
...
# Cependant, certains considèrent que l’utilisation de return rend le code plus lisible et explicite, 
...
# Ajouter la colonne 'reg' avec la valeur de 'territoire' 
# pour les lignes où 'echelle_geographique' vaut "Régional"
...
lire_et_transformer <- function(fichier, feuille) {
  read_xlsx(
    file = path(chemin_entree_agreg, fichier),
    sheet = feuille) |>
    as_tibble(.name_repair = make_clean_names) |>
    mutate(
...
Pour exporter un tableau de données dans R vers un fichier au format `.xlsx`, on peut utiliser `write_xlsx` du package `openxlsx2`.  

Les arguments principaux de `write_xlsx` sont :  

- x : le tableau de données à exporter (ou une liste de tableaux pour un export multi-feuilles)  
- file : le chemin et le nom du fichier de sortie, y compris l'extension .xlsx.  
- sheet (facultatif) : le nom de la feuille Excel dans laquelle écrire les données. Par défaut, "Sheet 1".  
- overwrite (facultatif) : indique s'il faut écraser un fichier existant. Par défaut, TRUE.  

On peut exporter plusieurs tableaux de données dans un même fichier Excel en utilisant `write_xlsx` avec une liste de tableaux de données.
On utilise alors les noms des éléments de la liste comme noms de feuilles.  

```{r , eval = FALSE}
library(openxlsx2)

# Export simple d'un seul tableau
write_xlsx(donnees, "export/fichier.xlsx")

# Export multi-feuilles à partir d'une liste de data.frames
# Liste nommée de tableaux de données
# Les noms de la liste servent de noms de feuilles
liste_tableaux <- list(
  'table1' = df1,
  'table2' = df2)

# Exporter la liste de tableaux de données dans un fichier Excel avec plusieurs feuilles
# à chaque élément de la liste va correspondre une feuille
write_xlsx(liste_tableaux, file = "fichier_multi_feuilles.xlsx")
```
...
# export simple d'un seul tableau
# Chargement du chemin 
chemin_sortie <- "S:/ETUDES/memento/sortie/"
fichier_sortie <- "abattoirs_volaille_pdl_2025.xlsx"

write_xlsx(volailles_sum,
           file = path(chemin_sortie, fichier_sortie),
           sheet = "abattoirs_volailles")

# export dans plusieurs feuilles
# avec un chemin construit via fs::path 
chemin_sortie <- "S:/ETUDES/memento/sortie/"
fichier_sortie <- "agri_bio_2025.xlsx"

# définir les noms de feuilles pour chaque tableau (data frame)
# les noms de la liste deviendront les noms des feuilles
...
write_xlsx(liste_tableaux, file = path(chemin_sortie, fichier_sortie))

```
https://janmarvin.github.io/openxlsx2/reference/write_xlsx.html
...
La fonction `summary` génère des statistiques résumant les données contenues dans les colonnes. Elle est souvent utilisée pour les variables numériques, mais elle peut aussi être utilisée pour les variables catégorielles.
Pour les colonnes numériques, on a accès à des informations clés sur la distribution : minimum/maximum, quartiles, médiane et moyenne. Cela permet de contrôler les ordres de grandeur et la cohérence des informations. 
La fonction `summary` fournit le nombre de valeurs manquantes s’il y en a.
...
### `mutate()` pour ajouter et modifier des variables
...
volailles_fr <- read_xlsx(file = path(chemin, fichier),
                          sheet = "res-France") |> 
  as_tibble(.name_repair = make_clean_names) |> 
  mutate(reg = "France") |> 
...
Le paramètre `.keep = "none"` indique à `mutate()` de supprimer toutes les colonnes existantes sauf celles explicitement mentionnées.
On ne conserve que les nouvelles colonnes qu'on vient de créer. Toutes les colonnes qui existaient déjà dans le tableau de données sont supprimées.
...
La fonction `separate` est utile lorsque nous avons une seule colonne contenant des valeurs composées que nous souhaitons diviser en plusieurs colonnes distinctes.
Elle prend trois arguments principaux :
...
# ajout colonne numéro de ligne
# creation d'une variable identifiant reprenant le numéro de ligne 
produits_laitiers <- read_xlsx(file = path(chemin_entree, fichier_entree), 
                               sheet = "produits laitiers", 
                               start_row = 12) |> 
  as_tibble(.name_repair = make_clean_names) 
...
La fonction `select()` permet de sélectionner les variables voulues par leur nom.
...
agri_bio_vegetal_fr_tab1_car <- agri_bio_vegetal_fr_tab1 |> 
  mutate(across(c(nombre_de_producteurs, surface_bio_et_en_conversion_en_ha,
                  ...
                  part_bio_de_la_sau_en_percent), 
                as.character)) |> 
  select(-part_bio_de_la_sau_en_percent)
```
...
otex <- read_xlsx(file = path(chemin_entree, fichier_entree),
                  sheet = "ETP - OTEX",
                  start_row = 7) |> 
  # Quand on passe une fonction comme argument à une autre fonction, on utilise la notation sans les parenthèses.
...
# type.convert pour transformer en numérique poids_produit_en_tec (caractère) 
# qui a comme séparateur décimal ","
viande_volaille_tec_2021 <- read_xlsx(file = path(chemin, fichier_saa),
                                      sheet = "Volailles finies produites") |> 
  as_tibble(.name_repair = make_clean_names) |> 
  filter(region %in% c("52  -  Pays de la Loire",
                       "98  -  Total France métropolitaine")) |> 
...
Avec `if_else` :   
`variable = if_else(condition(x), valeur_si_oui, valeur_si_non)` permet d'affecter *valeur_si_oui* ou  *valeur_si_non* à *variable* en fonction du fait que *x* répond à *condition*.  
...
La dernière clause `.default = "Autre"` permet d'assigner une valeur à toutes les lignes pour lesquelles aucune des conditions précédentes n'est vraie.
La condition `.default =`  indique la valeur par défaut à appliquer si aucune des conditions précédentes n'est vérifiée. Elle permet de gérer notamment les données pour lesquelles on a des valeurs manquantes ou des erreurs de frappe.
...
### comparaisons entre `case_match()`, `if_else()` et `case_when()`
...
Il existe quelques paramètres qui peuvent s’avérer utiles :  
    ...
    `sort =` : booléen qui permet, s’il est à TRUE, de classer le tableau par ordre décroissant des valeurs de la colonne `n`   
...
nb_exploit_2020 <- exploit_2020 |> 
              ...
              select(nom_dossier,
                     dep_2020 = siege_dep,
                     annee) |> 
  group_by(annee = factor(annee, levels = c(annee_ra_2020)), 
           dep_2020,
           .drop = FALSE) |> 
...
# qualca_ech : Part de la commercialisation sous AOP/AOC, IGP, 
# Label Rouge ou Spécialité Traditionnelle Garantie
...
Elle permet de définir des groupes de lignes (ou groupes de modalités) à partir des valeurs d’une ou plusieurs colonnes. 
...
lait_production_vache_large <- lait_production_vache_2 |> 
  ...
  adorn_totals(where = "row", name = "Pays de la Loire") |> 
  select(-starts_with("livraison_reg"))
...
lait_chevre <- read_xlsx(file = path(chemin_entree, fichier_entree),
                         sheet = "lait chevre", 
                         start_row = 8) |> 
  as_tibble(.name_repair = make_clean_names) |> 
  slice_head(n = 13) |> 
...
![](./images/widetolong.png){fig-align="center" width="80%"}

* **Transformation *long to wide* ** : 
  
![](./images/longtowide.png){fig-align="center" width="80%"}
...
### Utilisation de `names_pattern` dans `pivot_longer`
...
otex_regroupe <- otex_transp  |> 
  select(-ends_with('temp'), otex_4813, otex_4840, otex_5100, otex_5374) |> 
...
-   les fonctions `*_join(a, b, join_by)` du package `dplyr` permettent de joindre les tables `a` et `b` sur les variables de jointure précisées dans l'argument `join_by`.
...
![](./images/left_join.png){fig-align="center" width="60%"}
...
On précise explicitement quelles sont les identifiants qui permettent de faire ce lien entre les deux tables, à travers l'argument `join_by` :
...
![](./images/full_join.png){fig-align="center" width="50%"}
...
bio_pdl_test_doublons_var_ok <- bio_pdl |> 
  ...
  group_by(raisonsociale, numeropacage, numerobio, 
           libelleonab, ss_groupe_onab_libelle, nomoc) |>
  filter(n() > 1) |>
  arrange(raisonsociale, numeropacage, numerobio, 
          libelleonab, ss_groupe_onab_libelle, nomoc) |>
  ungroup()
...
bio_pdl_test_doublons_var_doubl <- bio_pdl |> 
  group_by(raisonsociale, numeropacage, numerobio, 
           libelleonab, ss_groupe_onab_libelle) |>
  filter(n() > 1) |>
  arrange(raisonsociale, numeropacage, numerobio, 
          libelleonab, ss_groupe_onab_libelle)|>
  ungroup()
...
### Tester sa jointure sur un full join

Une bonne pratique est de tester sa jointure une première fois dans un full join. On peut distinguer dans un full join trois types d’observations :
    - Observations présentes côté A seulement (type 1)  
...
<!-- 
Mise en italique des termes avec _italique_ ou *italique*.
-->


| **Type de jointure** | **Exemple** |
| :--------------------------------------------------: | :----------------------------------------------: |
| Jointure interne (_inner join_) | ![](images/Manipuler_donnees_dplyr/inner_join.png){fig-align="center" width="90%"} |
| Jointure à gauche (_left join_) | ![](images/Manipuler_donnees_dplyr/left_join.png){fig-align="center" width="90%"} |
| Jointure à droite (_right join_) | ![](images/Manipuler_donnees_dplyr/right_join.png){fig-align="center" width="90%"} |
| Jointure externe (_full join_) | ![](images/Manipuler_donnees_dplyr/full_join.png){fig-align="center" width="90%"} |
...
bio_2023 <- read_xlsx(file = path(chemin, fichier_2023), 
                      sheet = "Feuil1") |> 
  as_tibble(.name_repair = make_clean_names) |>
...
# On modifie le libellé espece avec la fonction gsub ou avec la fonction str_remove (c'est équivalent)
balsa_dep <- balsa_dep |> 
...
# str_replace(x, "nb_tetes_", "") : remplacer le préfixe "nb_tetes_" 
# par une chaîne vide dans les noms de colonnes.
...
```
## `str_pad` pour ajouter des caractères en début ou en fin de chaîne
...
Il est possible de remplir avant et / ou après la chaîne de caractères existant (avec l'argument `side`).
...
Les **expressions régulières** permettent la détection de motifs (patterns) sur des chaînes de caractères. 
Cette recherche textuelle sur motif (`pattern`) consiste à décrire ce qu'on cherche plutôt que comment chercher.
...
str_replace_all(`"anormale"`, `"^a"`, `""`)  # un `a` mais seulement en début de chaîne
# [1] "normale"  
...
- Rechercher des caractères appartenant à des groupes particuliers
  ...
  - une liste de caractères : la séquence `[…]` et son contraire `[^...]`
    str_replace_all("Concrètement", "[oe]", "")   
    # un 'o' ou un 'e'  
    ## [1] "Cncrènt"
...
### Séquence d’échappement des caractères spéciaux et `raw string`
...
textes <- c("14/07/1789", "J'ai 10 ans.", "Une date importante est le 01/01/2000")
...
bio_anim2_pdl_an_evol <- bio_anim2_pdl_an_evol |>
  mutate(
    ...
    nb_eleveurs_2023_2010_evol_an = ((nombre_deleveurs_2023 / nombre_deleveurs_2010)**(1 / 13) - 1) * 100,
    solde_eleveurs_2023_2010_evol_an = (nombre_deleveurs_2023 - nombre_deleveurs_2010) / 13,
    part_bio_percent_2023_2010_evol_an = (part_du_bio_en_percent_2023 - part_du_bio_en_percent_2010) / 13,
...
<center>![Schéma général d'une fonction](images/principe_fonction.png)</center>
...
En réalité, l'utilisation de `return()` n'est pas obligatoire : une fonction retourne automatiquement le résultat de la dernière instruction qu'elle exécute. Mais certains considèrent qu'il est souvent préférable de l'utiliser pour rendre le code plus lisible.
...
=> bloc d'instructions de la fonction. Le *corps* de la fonction, situé à l’intérieur d’un bloc `{}`, est la partie de la fonction qui permet d’appliquer les manipulations voulues pour obtenir le résultat à retourner.  
On peut commencer par réaliser une instruction simple sur quelques données. Une fois que ça fonctionne pour un élément, il sera plus simple de généraliser. 

Étape 2 : identifier ce que sera l’entrée de la fonction  

Étape 3 : identifier ce qui varie dans le code
=> arguments (éléments *variables*) de la fonction (avec ou sans valeur par défaut). Il peut s’agir de l’objet qui sera transformé par la fonction (par exemple un dataframe) ou d’un paramètre utilisé dans la fonction (par exemple un entier).  

Étape 4 : définir ce que renvoie la fonction (sortie de la fonction)
=> dernier objet ou objet retourné par return(). En effet, return est facultatif. Certains considèrent que l'ajouter permet de rendre le code plus lisible. 

Étape 5 : définir ce que fait la fonction
=> nom de la fonction. On conseille de donner un verbe comme nom de fonction (par exemple calculer_moyenne, renommer_colonnes).  

Étape 6 : tester et vérifier la fonction  
...
Les objets créés lors de l'exécution d'une fonction sont à l'inverse des **objets locaux** : ils n'existent qu'à l'intérieur de la fonction et pour la durée de son exécution. 
Les variables locales ont une portée limitée à une fonction ou à un environnement spécifique. Elles existent dans des environnements à portée limitée (comme un environnement d’exécution).  
...
![](images/structure_if.svg){fig-align="center" width="80%"}
...
![](images/structure_if_else.svg){fig-align="center" width="80%"}
...
### Différence entre `if` / `else` et `if_else`
...
Exemple de fonction avec `if` / `else`
...
calculer_montants_par_departement <- function(annee) {
  ...
  # get() prend en entrée une chaîne de caractères représentant le nom d'un objet et retourne cet objet
...
![](images/structure_for.svg){fig-align="center" width="90%"}
...
La boucle `while` permet de répéter une opération tant qu’une condition est vérifiée.
...
![](images/structure_while.svg){fig-align="center" width=90%}
...
### Principe de`map` et ses variantes
...
- avec une fonction anonyme  
    - définie avec la syntaxe concise `\(x)` et `x` : les éléments successifs de la liste sont alors identifiés avec `x`  
      exemple map(..., \(x) x |> summarise(moyenne = mean(population)))
...
`x` est une variable positionnelle qui représente l'élément courant.
...
# na.strings : spécifie comment les valeurs manquantes doivent être représentées dans le fichier Excel 
write_xlsx(dfs_nb_tetes_par_groupe_production_clean,
...
### Exemples de programmation avec le *tidyverse* 
...
Ajouter une nouvelle ligne basée sur un ratio entre lignes : 1^re^ méthode
...
Ajouter une nouvelle ligne basée sur un ratio entre lignes : 2^e^ méthode
...
# Utiliser la fonction
# modalite_1/2/3 avec guillemets dans l'appel de la fonction 
# car utilisés comme chaine de caractères dans filter()
...
## Paramétrer une chaîne de caractères avec le package `glue`
...
- éviter le mélange de majuscules et minuscules dans les noms des objets et donc utiliser la [snake case](https://fr.wikipedia.org/wiki/Snake_case) (minuscules et underscores) pour les noms de variables et de fonctions (ma_variable, ouvrir_ra()) ; 
...
Ces 4 sites servent de source pour alimenter le présent [document](https://christophefouchard.github.io/pratique_R/pratique_R.html).
...
On peut convertir un vecteur d'un type à un autre avec les fonctions de forme as.XXX :   
`as.character()`, `as.numeric()`, `as.factor()` et `as.logical()`.  
...
Un *vecteur* est un ensemble d’une seule dimension de 1 ou plusieurs éléments du même type.
...
La fonction `seq()` (comme séquence) génère une séquence de valeurs numériques. Elle prend comme paramètre un point de départ (1^er paramètre from =), un point d’arrivée (2e paramètre to =) et le pas (3^e^ paramètre by =) qui indique l’écart régulier souhaité entre deux valeurs successives.
...
- **Objectif :**
  - Vérifie si deux objets R sont strictement identiques. 
...
## Travailler avec les valeurs manquantes `NA`
...
# Pour les noms de colonnes commençant par des chiffres, 
# on doit les entourer de backticks (`) pour les utiliser
nb_benef_montants_pac_2020_2023_secret2 <- nb_benef_montants_pac_2020_2023_secret1 |>
  mutate(
    # Secret 2020
    `44_2020` = if_else(indicateur == "dont_nb_benef_aide_legum_fourrage_deshy", "Secret", `44_2020`),
    `53_2020` = if_else(indicateur == "dont_nb_benef_aide_legum_fourrage_deshy", "Secret", `53_2020`),
    # Secret 2021
    `44_2021` = if_else(indicateur == "dont_nb_benef_aide_legum_fourrage_deshy", "Secret", `44_2021`),
    `49_2021` = if_else(indicateur == "dont_nb_benef_aide_semences_legum_fourrageres", "Secret", `49_2021`),
    # Secret 2022
    `44_2022` = if_else(indicateur == "dont_nb_benef_aide_legum_fourrage_deshy", "Secret", `44_2022`),
    `53_2022` = if_else(indicateur == "dont_nb_benef_aide_legum_fourrage_deshy", "Secret", `53_2022`),
    `49_2022` = if_else(indicateur == "dont_nb_benef_aide_semences_legum_fourrageres", "Secret", `49_2022`),
    `72_2022` = if_else(indicateur == "dont_nb_benef_aide_semences_legum_fourrageres", "Secret", `72_2022`)
  )
...
On cherche par exemple à déterminer en quelle position se trouve la région d'intérêt, comme dans le fichier   https://draaf.pays-de-la-loire.agriculture.gouv.fr/IMG/xlsx/annexe_regionale_2025.xlsx  
présent dans  
https://draaf.pays-de-la-loire.agriculture.gouv.fr/dossier-territorial-a1657.html.  
...
R est prêt à exécuter une commande lorsque le chevron > est affiché dans la console. Si le chevron n'apparaît pas, c'est qu'une commande est incomplète. Appuyer sur `Echap` pour sortir de cette commande.
...
```
En effet, la valeur associée à .default = doit être de même type que la variable créée (ici actif), soit de type caractère.
En associant, .default = à un élément de type "caractère", cela a résolu le problème.
