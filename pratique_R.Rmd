...
.libPaths()
# "/var/data/nfs/CERISE/00-Espace-Personnel/prenom.nom/R/x86_64-pc-linux-gnu-library/4.2"

# éléments du répertoire :
list.files("~/CERISE/00-Espace-Personnel/prenom.nom/R/x86_64-pc-linux-gnu-library/4.2")
...
row.names(x = installed.packages(
                        lib.loc = c("~/CERISE/00-Espace-Personnel/prenom.nom/R/x86_64-pc-linux-gnu-library/4.2")))
```
...
remove.packages(pkgs = "cli", 
                lib = c("~/CERISE/00-Espace-Personnel/prenom.nom/R/x86_64-pc-linux-gnu-library/4.2"))

# suppression de l'ensemble des packages pour repartir de zéro
remove.packages(pkgs = row.names(
  x = installed.packages(lib.loc = c("~/CERISE/00-Espace-Personnel/prenom.nom/R/x86_64-pc-linux-gnu-library/4.2"))),
                lib = c("~/CERISE/00-Espace-Personnel/prenom.nom/R/x86_64-pc-linux-gnu-library/4.2"))
...
# * removing ‘/var/data/nfs/CERISE/00-Espace-Personnel/prenom.nom/R/x86_64-pc-linux-gnu-library/4.2/tidyverse’
# Warning in install.packages :
...
entetes_vol_2023 <- read_xls(path = paste0(chemin_entree,
                                           chemin_2023,
                                           fichier_entree_2023), 
...
vol_2023 <- read_xls(path = paste0(chemin_entree,
                                   chemin_2023,
                                   fichier_entree_2023), 
                     sheet = "res_nouvelle_region", 
                     skip = 23,
...
# On restreint les lignes à lire avec skip et n_max
vol <- read_xls(path = paste0(chemin_entree,
                              fichier_entree), 
...
list_eff_annees <- map(annees, function(annee) {
  ...
  df <- read_csv2(file = paste0(chemin_entree_avant_2010, fichier_entree_eff_annee)) |> 
    as_tibble(.name_repair = make_clean_names)
...
```{r , eval = FALSE}
# Utilisation de la fonction recode pour renommer les modalités
# La première entrée de la fonction est la variable que l'on souhaite recoder.
# La deuxième entrée est une liste de remplacements. 
# Chaque élément de la liste doit correspondre à une modalité de la variable à recoder.
# La troisième entrée est facultative. Elle permet de spécifier le type de remplacement à effectuer. 
# Les valeurs possibles sont :
# "replace" : la valeur de la modalité est remplacée par la nouvelle valeur.
# "if_else" : la valeur de la modalité est remplacée par la nouvelle valeur si la condition est remplie.
# "case_when" : la valeur de la modalité est remplacée par la nouvelle valeur en fonction d'une série de conditions.
# Ici, le type de remplacement est implicitement défini sur "replace".
viande_volaille_avant_2010_4 <- viande_volaille_avant_2010_4 |>
  mutate(type_grandeur = recode(
    type_grandeur,
...
# vérifie que la chaîne de caractères recherchée est suivie d'une autre chaîne de caractères spécifique, 
# sans inclure cette chaîne de caractères dans le résultat de la recherche.
eff_bovins_2024 <- read_csv2(file = paste0(chemin_bdni, eff_an_2025, fichier_01_2025)) |> 
